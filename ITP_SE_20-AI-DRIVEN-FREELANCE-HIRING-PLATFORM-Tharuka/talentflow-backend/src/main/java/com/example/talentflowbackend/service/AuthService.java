...
public class AuthService {
    ...
    public AuthResponse switchRole(String authHeader, String requestedRoleStr) {
        try {
            ...
            Role activeRole = parseRole(requestedRoleStr);
            if (activeRole == null || !user.hasRole(activeRole))
                return AuthResponse.builder().message("You do not have the requested role for this account.").build();

            String jwtToken = jwtService.generateToken(user, activeRole);
            return AuthResponse.builder()
                    .token(jwtToken).message("Switched to " + activeRole.name() + " role.")
                    .role(activeRole.name())
                    .roles(user.getRoles().stream().map(Role::name).toList())
                    .user(user).build();
        } catch (Exception e) {
            return AuthResponse.builder().message("Role switch failed: " + e.getMessage()).build();
        }
    }
